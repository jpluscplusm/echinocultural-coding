...
bootstrap_files := $(hashbang) $(main_func) functions/example_functions $(main_script) $(tests_make)
$(bootstrap_files):
	@echo Creating $@
	@echo $(ARCHIVE) | base64 --decode | cpio --quiet -d -i $@
bootstrap: $(bootstrap_files) ## Creates a script, function & test skeleton structure
bootstrap-archive:
	echo $(bootstrap_files) | tr ' ' \\n | cpio --quiet -o | gzip -9c | base64
